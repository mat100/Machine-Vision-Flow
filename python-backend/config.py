...
class CameraConfig(BaseSettings):
    ...
    default_camera: str = Field(
        default=CameraConstants.DEFAULT_CAMERA_ID,
        description="Default camera ID"
    )
    ...
    stream_fps: int = Field(
        default=CameraConstants.MJPEG_FPS,
        ge=1,
        le=60,
        description="MJPEG stream FPS"
    )
    stream_quality: int = Field(
        default=CameraConstants.MJPEG_QUALITY,
        ge=1,
        le=100,
    ...
    test_image_width: int = Field(
        default=CameraConstants.TEST_IMAGE_WIDTH,
        ge=100,
        le=4096,
        description="Test image width"
    )
    test_image_height: int = Field(
        default=CameraConstants.TEST_IMAGE_HEIGHT,
        ge=100,
        le=4096,
        description="Test image height"
    )
...
class TemplateConfig(BaseSettings):
    ...
    allowed_formats: List[str] = Field(
        default=TemplateConstants.ALLOWED_FORMATS,
        description="Allowed template image formats"
    )
...
class VisionConfig(BaseSettings):
    ...
    min_contour_area: int = Field(
        default=VisionConstants.MIN_CONTOUR_AREA_DEFAULT,
        ge=1,
...
class APIConfig(BaseSettings):
    ...
    api_version: str = Field(
        default=APIConstants.API_VERSION,
...
class SystemConfig(BaseSettings):
    ...
    log_level: str = Field(
        default=SystemConstants.LOG_LEVEL_DEFAULT,
    ...
    data_dir: str = Field(
        default=SystemConstants.DATA_DIR,
        description="Data directory path"
    )
    temp_dir: str = Field(
        default=SystemConstants.TEMP_DIR,
